...
export default function ETADisplay({ selectedRoute }: ETADisplayProps) {
  ...
  useEffect(() => {
    // Fetch real ETA data based on selected route
    const fetchETAData = async () => {
      ...
      try {
        ...
        const transformedStops = predictions.map((pred: any) => ({
          id: pred.stop_id,
          ...
          status: (pred.delay_minutes > 5 ? 'delayed' : pred.delay_minutes < -2 ? 'early' : 'on-time') as 'on-time' | 'delayed' | 'early',
    ...
    const interval = setInterval(fetchETAData, 30000);
